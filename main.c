...
int main(int argc, char *argv[])
{
    ...
    printf("============PUNTO C============\n");
    sparsify(A, rows, cols);
    printf("============PUNTO D============\n");
    //Lunghezza del mio nuovo array
    int array_length;
    double *array = collect(A, rows, cols, &array_length);
    printf("============PUNTO E============\n");
    printf("Scrivo il nuovo file con i %d valori del nuovo array creato\n", array_length);
    writeToFile(array, outputFilename, &array_length);
...
void sparsify(double ***matrix, int rows, int cols)
{
    ...
    for (int i = 0; i < rows; i++)
    {
        ...
        // calcolo la media
        med = sum / cols;
        // Riscorro la riga per togliere i valori che sono inferiori alla media, mettere a null i puntatori e liberare la memoria
